...
| 2026-09-22 | both machines: laptop (melo-lap-001) and desktop (melo-desk-001), Windows 11 + Ubuntu-26.04 (WSL2), build `1.0.1+1a55f7f` | v1.0.1 install bump, and R2's own named check | pass | Both sides of both machines moved from `1.0.0+2a30095` to `1.0.1+1a55f7f` in one window, installed by `chezmoi apply` from the `v1.0.1` release assets after melodic-software/dotfiles#823 pinned them. Each side was stopped before its binary was replaced, because the version compare is ordinal and a half-applied bump refuses every hand-off. **The follower-log wrapper is installed**: `claude-code-account-rotation-follower-log` (9094 B, `#!/bin/sh`), beside the follower binary under exactly that name, which is the path `WslDistributionPeerHost` derives for itself; its absence fails silently. **R2's own check, obtainable for the first time and never run before on any machine**: after the leader started the side, `grep -c "auth login" <appDataDirectory>/follower.log` printed **`0`** on each machine, against a log the wrapper had truncated and written that start (first line `Now listening on: http://localhost:48212`). Read that result for exactly what it covers: **the wrapper truncates `follower.log` on every start, so a `0` speaks for the current follower invocation, not for the machine's whole history.** The cumulative claim that neither distro has ever logged in still rests on the two earlier rows, whose evidence is the fingerprint identity of the handed-over pair and the follower's 404 login route. What is new here is that the criterion's own instrument exists and reads clean, where before there was no log to grep at all. **R5**: `check-single-holder.sh --root` over the four real roots read `files=10 distinct=10 duplicates=0 recovery=0 quarantine=0` on each machine after the upgrade. Also observed: neither leader starts its side by itself after a restart, so the side reads `offline` until one `POST /api/sides/{side}/start`; and the leader is a console host with no windowless mode of its own, so a hand launch needs `-WindowStyle Hidden` plus redirection to keep writing `leader.log` (the logon task in melodic-software/provisioning#552 owns that shape) |
