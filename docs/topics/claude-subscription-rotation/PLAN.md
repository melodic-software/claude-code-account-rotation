...
- Refreshing a parked pair's token renews its four-week login (observed behavior of the credential store; revisit trigger: an account expiring despite weekly refreshes). **Correction note 2026-09-21 (#70): this assumption is false.** Measured 2026-09-19 (`docs/topics/cross-os-rotation/design/design-resolution.md` section 2): the 28-day login window is **fixed** and a refresh does not slide it — `refresh_token_expires_in` counted down to 2 156 885 s against 32 023 s elapsed between two passes, so a refresh moved the recorded expiry by about **+1.3 s**, not by 28 days. Re-measured live during phase 5's acceptance: the live account's `loginExpiresAt` moved **+98 ms** across a leader restart that refreshed it. Weekly refreshes therefore do **not** keep a login alive; every account still needs its own browser login once per 28 days. This is the whole reason one token family per account per machine is worth a shared store — two families would mean 20 re-logins per machine per month instead of 10, forever.
...
| Cross-volume hand-off to the other side of the machine by staged copy, fingerprint verify, promote, delete, under a journal and an export gate (**amendment note 2026-09-21, #70**: supersedes the row above for the WSL lane only) | Not rejected — **adopted**, because the row above forbids the only shape a two-volume machine allows, and the alternative of a second token family per account costs 10 extra browser logins per machine every 28 days once the fixed login window is accounted for. The failed-delete objection is answered by the journal, which finishes or unwinds after a crash, and by the export gate: the follower exports and stops, the leader reads the exported file natively on the store's own volume and compares fingerprints, and only a commit lets the swap that destroys the outgoing pair's last local copy proceed | Reverse it if a cross-side rename ever becomes possible (one volume for both sides), or if the 28-day window is measured to slide after all, which would make two families cheap |
