...
- A live credential file that had a parsed pair and then lacks an access token or a refresh
  ...
  logged out at that UTC time, and Switch is not offered while that stands. A missing file and
  a half-written file do not record a logout. A later pair clears the record, so a following
